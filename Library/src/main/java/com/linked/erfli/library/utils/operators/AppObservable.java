...
public final class AppObservable {
    ...
    public static <T> Observable<T> bindActivity(Activity activity, Observable<T> source) {
        /*
        WrapComponent wrap = null;
        if (activity instanceof LinkedinActivityBase) {
            wrap = ((LinkedinActivityBase) activity).wrapComponent;
        }else if (activity instanceof LinkedinActionBarActivityBase) {
            wrap = ((LinkedinActionBarActivityBase) activity).wrapComponent;
        }
        if (wrap != null) {
            return source.observeOn(mainThread()).compose(wrap.bindToLifecycle());
        }
        */
        return source.subscribeOn(Schedulers.from(ThreadPool.executor)).observeOn(mainThread()).lift(new OperatorConditionalBinding<T, Activity>(activity, ACTIVITY_VALIDATOR));
    }
    ...
    public static <T> Observable<T> bindFragment(Object fragment, Observable<T> source) {
        /*
        if (fragment instanceof LinkedinFragmentBase) {
            WrapComponent wrap = ((LinkedinFragmentBase) fragment).wrapComponent;
            return source.observeOn(mainThread()).compose(wrap.bindToLifecycle());
        }
        */
        final Observable<T> o = source.subscribeOn(Schedulers.from(ThreadPool.executor)).observeOn(mainThread());
